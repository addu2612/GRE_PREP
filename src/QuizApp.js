...
const QuizApp = () => {
  ...
  const generateQuestion = () => {
    ...
    while (incorrectOptions.length < 3) {
      ...
      if (option.word !== correctAnswer.word && !incorrectOptions.includes(option)) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4">
      <h1 className="text-4xl font-bold mb-8 text-blue-600">GRE Vocabulary Quiz</h1>
      {currentQuestion && (
        <div className="mb-8">
          <h2 className="text-2xl font-semibold mb-4">{currentQuestion.meaning}</h2>
          <div className="grid grid-cols-2 gap-4">
            {options.map((option) => (
              <button
                key={option.word}
                onClick={() => handleAnswer(option.word)}
                className="px-6 py-3 bg-blue-500 text-white rounded-lg shadow-md font-semibold"
              >
                {option.word}
              </button>
            ))}
